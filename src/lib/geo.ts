/** FIPS state-code helpers so every response carries a friendly state name/abbr. */

const STATES: Record<string, { abbr: string; name: string }> = {
...
export function stateAbbr(fips: string | undefined): string | undefined {
  return fips ? STATES[fips]?.abbr : undefined;
}

export function stateName(fips: string | undefined): string | undefined {
  return fips ? STATES[fips]?.name : undefined;
}
